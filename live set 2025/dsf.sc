...
(
    SynthDef(\fftStretch_magAbove_stereo, {
        ...
        chains = 2.collect { |i|
            ...
            accumChain = LocalBuf(\fftSize.kr(2048));
...
(
    ...
    Ndef(\dsf, {
        ...
        Splay.ar(sig, \spread.kr(1));
            ...
            //     // \rate, 10.68,
            //     \rate,  Pstep([4, 2, 1, 2, 8] * 75/60, 2, inf),
            //     // \rate, tempo * 8,
...
(
    ...
    x = {
        ...
        player.label;
            ...
            //     // \tFreq, ~slider.(0).linexp(0,1,5,20000).lag(0.1),
            //     // \rate, 10.68,
            //     \rate, tempo * Pstep([4, 2, 1, 2, 8] * 2, 2, inf),
            //     // \rate, tempo * 8,
            //     // \rate, tempo * Pstep([8, 16], 0.5, inf),
            ...
            Ndef(\dsf)[999] = \pset -> Pbind(
                ...
                // \tFreq, ~slider.(0).linexp(0,1,5,20000).lag(0.1),
                // \rate, 10.68,
                \rate,  Pstep([4, 2, 1, 2, 8] * tempo, 2, inf) / 4,
                // \rate, tempo * 8,
                // \rate, tempo * Pstep([8, 16], 0.5, inf),
                \fm, ~slider.(2).linexp(0,1,0.001,12000),
                \curve, ~slider.(3).linlin(0,1,-4,4),
...
(
FluidBufNMF.processBlocking(s,~src1,bases:~src1Bases,activations:~src1Activations,components:20, action:{"Analysed Source 1".postln});
FluidBufNMF.processBlocking(s,~src2,bases:~src2Bases,activations:~src2Activations, components:20, action:{"Analysed Source 2".postln});
)
