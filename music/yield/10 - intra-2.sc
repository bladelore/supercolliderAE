...
(
    ...
    Pdef(\kick, 
        Pbind(
            ...
            \atk, 0.04,
            \dec, Pkey(\groupdelta).lincurve(0, 1, 0.1, 1),
            // \dec, 0.5,
            // \rel, 0.01,
            ...
            \pan, 0.0,
            \amp, 1,
            // \amp, Pkey(\cycledelta).lincurve(0, 1, 1, 0.7),
...
(
    ~recorders = ~recordBuses.value(
        ~busArr,
        Platform.recordingsDir +/+ "intra_new_kick2/%.wav"
    );
)
...
x = {
    ...
    kick = (
        Pdef(\kick) <>
        // ~filterBeat.(key: Pkey(\groupcount), beat:[8], reject: 1, mod: 8) <>
    ...
    Pbindef(\cut, 
        ...
        \sliceStart, Pstep(Pseq([149, 200], inf), 4, inf),
        \rel, Pkey(\dur) * 0.25,
        \pitchMix, ~pmodenv.(Pseq([0, 1], inf), 4, 1, \sine),
        // \pitchMix, 1,
        \pitchDispersion, ~pmodenv.(Pseq([0.1, 0.4], inf), 8, 1, \linear),
    );
    ...
    break = (
        Pbind(\out,
            Pstep(
                Pseq(
                [[~mainout, ~sineTracker, ~miVerb, ~convolve_A, ~break_out],
                [~sineTracker, ~fftStretchLive, ~miVerb, ~convolve_A]], inf)
            , 16, inf)
        ) <>
        Ppar([
            Pdef(\cut_L) <> Pdef(\cut), 
        ...
        // ~filterBeat.(key: Pkey(\groupcount), beat:[1], reject: 1, mod: 6) <>
        Pdef(\p1)
    ...
    Pbindef(\cut, 
        // \out, [ ~fftStretchLive, ~sineTracker],
        \pitchMix, ~pmodenv.(Pseq([0, 1], inf), 4, 1, \sine),
        \rate, Pstep(Pseq([0.5, 0.25], inf), 1.5, inf),
        \sliceStart, Pstep(Pseq([5, 25, 50, 75] + 100, inf), 8, inf),
        // \rel, Pkey(\dur) * 1
        \rel, ~pmodenv.(Pseq([2, 0.25], inf) * Pkey(\dur), 8, 1, \sine)
    );
    ...
    sineTracker = (
        Pbind(
            \feedback, ~pmodenv.(Pseq([0.5, 0], inf), 8, 1, \sine),
            \env_atk, 0.01,
            \env_rel, 0.1,
            \freqLag, 0.1,
            \ampLag, 0.1,
    ...
    Pbindef(\cut, \sliceStart, Pstep(Pseq([149, 100], inf), 4, inf)).quant([1,0]);
    Pbindef(\cut, \rel, Pkey(\dur) * 0.25).quant([1,0]);
    Pbindef(\cut, \rate, 1).quant([1,0]);

    (   
        Pbind(\out, [~mainout, ~miVerb, ~prism_out]) <>
        Pdef(\prism) <>
