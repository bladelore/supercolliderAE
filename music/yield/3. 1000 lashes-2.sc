...
(
    ~busArr = [
        ...
        ~bridge_out=Bus.audio(s,2),
        ~rope_out=Bus.audio(s,2),
        ~drum_out=Bus.audio(s,2),
        ~chain_out=Bus.audio(s,2),
    ]
)
...
(
    ~recorders = ~recordBuses.value(
        ~busArr,
        Platform.recordingsDir +/+ "1k Lashes1/%.wav"
    );
)
...
Pspawner({| sp |
   ...
   drum = sp.par(
    ...
    //             \timeDispersion, 0.05,
    //             \out, [~mainout, ~convolve_B, ~rope_out],              
    //         )
    ...
    bridge = sp.par(
        Pdef(\bridge,
            Pmono(
                ...
                // \dec, 0.1,
                \dec, 2,
                \out, [~mainout, ~convolve_B, ~bridge_out]
            )
            <> Pdef(\p1)
        )
    );
