...
(
    ...
    // ~analyzeSlices.("/Users/aelazary/Desktop/Samples etc./Old sample lib/Prime Loops The Temple Of Breaks (MULTIFORMAT)/WAV_ACID_The_Temple_Of_Breaks/The Temple Of Breaks/Drum Loops/Tribe_Break_177.wav", ~sliceBuf, 0.1, \centroid);
...
(
    ~busArr = [
        ...
        ~roar_out=Bus.audio(s,2),
        ~filter_out=Bus.audio(s,2),
        ~reverb_out=Bus.audio(s,2),
        ~sineTracker_out=Bus.audio(s,2),
        ~fieldRec_out=Bus.audio(s,2),
        ~backyard_out=Bus.audio(s,2),
...
(
    ~recorders = ~recordBuses.value(
        ~busArr,
        Platform.recordingsDir +/+ "mirror_sequence3/%.wav"
    );
)
...
x = {
    ...
    // load vst
    ~serumVst = VSTPluginController(Synth(\vsti, [\out, [~mainout]])).open("Serum");
    0.5.wait;
    ~serumVst.loadPreset("distortedHit");
    0.5.wait;
    ...
    // sample = PfadeIn(Pmono(\samplePlayer, \buf, ~introSample, \out, [~mainout, ~sineTracker, ~fieldRec_out]), 8);
    ...
    cut3 = 
        Pbind(\out, [~roar, ~miVerb, ~sineTracker]) <>
        Pdef(\cut3) <>
    ...
    Pdef(\p1,
        ~makeSubdivision.(
            // PlaceAll([1, 1, 1, 1, 1.5, Rest(1.5), Rest(1)], inf),
    ...
    // Pbindef(\cut1, \sliceStart, Pseq([20, 15], inf).stutter(6));
    // Pbindef(\cut1, \rel, 0.25);
    ...
    cut3 =
        Pbind(\out, [~mainout]) <>
        Pdef(\cut3) <>
        ~filterBeat.(key: Pkey(\cyclecount), beat:[1], reject: 0, mod: 3) <>
        ...
        PfadeIn(
            Pdef(\backyard,
                    Pmono(\fftStretch_mono,
                        ...
                        // \filter, 1,
                        \len, 0.05,
                        \out, [~mainout, ~miVerb, ~backyard_out]
                    )
                )
        , 16)
    ).play(t);
    ...
    roar = (
        Pbindf(
            ...
            \amp, 1.0,
            // \group, ~morphGroup,
            \out, [~mainout, ~sineTracker, ~roar_out]
        )
    ).play(t);
